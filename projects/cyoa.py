...
def greet() -> None:
    ...
    global player
    player = input(f"Hey there, my name is Chad!{GENIE_EMOJI} \nWhat is your name? ")
...
def continue_game() -> None:
    """This procedure lets the player exit the game after tellig them their points."""
...
def slot_machine(money: int) -> int:
    """This is the unfair gambling machine!"""
    global points
    global wager
    ...
    if int(wager) > int(points):
        adjust_wager: str = input(f"Sorry, you currently have {points} dollars. Please input an integer less than or equal to {points}. ")
        adjust_wager = wager
    print("Good luck!")
    if bet_level == "high":
        high(wager)
...
def high(bet: int) -> int:
    ...
    won_points: int = 0
    won_points = int(int(won_points) + int(prize))
    wager = int(won_points)
    return wager
...
def main() -> None:
    ...
    while play_again == "yes" and points > 0:
        ...
        if points < 0:
            ...
            exit()
        play_again: str = input(f"Your current balance is {points} dollars. Type 'yes' to try again! Otherwise, type 'no'. ")
